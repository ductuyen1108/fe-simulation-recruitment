...
const traits = [
  {
    ...
    color: 'blue',
    align: 'right',
    img: '/assets/personality-test/step1_illustration.svg',
    description:
  ...
  },
  {
    ...
    color: 'yellow',
    align: 'left',
    img: '/assets/personality-test/step2_illustration.svg',
    description:
      '外向型の人はグループで行う活動が好きで、人との交流を大切にします。また、自分の熱意や興奮感を表に出す傾向があります。'
  },
  {
    ...
    color: 'green',
    align: 'right',
    img: '/assets/personality-test/step3_illustration.svg',
    description:
      '外向型の人はグループで行う活動が好きで、人との交流を大切にします。また、自分の熱意や興奮感を表に出す傾向があります。'
  },
  {
...
const PersonalityTraits = () => {
  ...
  const selectedLabel = active.align === 'left' ? active.leftLabel : active.rightLabel
  const selectedPercent = active.value
  const handleClick = () => {}
  return (
    <Element name='personality-traits' className='space-y-6'>
      ...
      <div className='space-y-6'>
        <div className='flex items-center gap-3'>
          ...
          <h2 className='text-[30px] text-neutral-600  font-semibold'>性格特性</h2>
        </div>
        <div className='border border-neutral-300 rounded-[8px]'>
          <div className='flex w-full p-4'>
            <div className='w-full flex flex-col'>
              {traits.map((t, i) => (
                <div key={t.id} onMouseEnter={() => setActiveIdx(i)}>
                  <TraitBar
                    ...
                    color={t.color}
                    align={t.align}
                    className={i === activeIdx ? 'bg-neutral-50' : ''}
                  />
                </div>
              ))}
            </div>
            <div className='bg-neutral-50 rounded-tr-[8px] rounded-br-[8px] rounded-bl-[8px] p-4 flex flex-col gap-1 max-w-[150px] md:max-w-[215px]'>
              <div className='text-center w-full'>
                <p className='text-xs text-neutral-600'>{active.title}</p>
                <span className='text-neutral-600 font-semibold'>
                  <p className=''>{selectedPercent}%</p> {selectedLabel}
                </span>
              </div>
              <img src={active.img} alt='' />
              <p className='text-xs text-neutral-600'>{active.description}</p>
            </div>
          </div>
          <div className='bg-neutral-50 p-4 rounded-bl-[8px] rounded-br-[8px] flex flex-col gap-2'>
            ...
            <Link to={'unlock-all-results'} spy smooth offset={-100} duration={400} spyThrottle={100}>
              <Button
                onClick={handleClick}
                className='flex items-center px-4 py-2 gap-2 rounded-[36px] w-fit text-xs  font-semibold'
              >
                <Send2 size='16' /> 結果を送る
              </Button>
            </Link>
